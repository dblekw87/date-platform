...
1. **토스 랭킹 API 429** — 토큰 발급은 성공하는데 `/api/v1/rankings`가 계정 쿼터를
   반환합니다. 미국 주도주는 `providers/market.mjs`의 Yahoo Finance 스크리너로
   대체했으므로 보드는 정상 동작하고, 토스 어댑터는 상태만 보고합니다.
   토스를 되살리려면 콘솔에서 한도를 확인해야 합니다.
