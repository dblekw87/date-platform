...
2. **커뮤니티 검색 미구현** — `/community`의 검색창은 아직 동작하지 않습니다.
3. **조회수 미집계** — `community_posts.view_count`를 올리는 코드가 없습니다.
4. **댓글 삭제 없음** — 목록/작성/본인 수정만 있고 삭제 엔드포인트는 없습니다.
   저장소 전체에 DELETE를 쓰는 곳이 없어 맞춰뒀습니다.
