...
export type LimitPairDto = {
  ...
  theme: string;
  /**
   * 짝 두 종목을 **뺀 나머지** 테마 회원이 오늘 얼마나 앞섰는가 — 모집단 평균
   * 대비 초과입니다. 남은 회원이 없으면(2종목짜리 테마) null이고 themeMembers는 0입니다.
   *
   * 짝은 공유 테마가 하나라도 있으면 성립하므로 한 종목이 여러 카드의 1등주로
   * 동시에 나옵니다. 2026-08-31 사토시홀딩스는 드론과 건강기능식품 두 카드에
   * 5분 간격으로 올라왔는데, 편입이 넷이라 틀린 카드는 없지만 +30%로 민 힘은
   * 하나였습니다. 이 숫자가 그것을 가릅니다.
   *
   * **짝 자신은 평균에서 뺍니다.** 넣으면 "이 두 종목이 올랐다"를 "테마가 올랐다"로
   * 다시 말하는 순환입니다. 2026-09-01 알림 넷을 다시 재보니 짝을 빼자 전부 부호가
   * 뒤집혔습니다(마이크로 LED +5.93 → -2.96, 4종목 중 2개가 짝이었음). 빼도 원래
   * 목적은 살아남습니다 — 사토시홀딩스는 건강기능식품 +6.47 → +1.93, 드론
   * +2.68 → -3.62으로 승자가 그대로면서 부호까지 갈립니다.
   *
   * **장중과 마감 뒤 기준이 다릅니다.** 장중에는 분봉이 닿는 종목만 모집단이라
   * 오른 회원만 세고(그래서 값이 큽니다), 마감 뒤에는 전 종목 일봉이라 테마
   * 전체가 들어옵니다. 같은 테마가 장중 +6.47%p(12종목) → 마감 +1.64%p(44종목)로
   * 바뀝니다. 둘 다 맞는 값이고 모집단이 다른 것이라, themeMembers를 같이 보아야
   * 합니다. 종목에 붙는 테마 라벨 판정도 같은 방식으로 갈리므로 카드와 라벨은
   * 서로 어긋나지 않습니다.
   */
  themeMove: number | null;
  /** 짝을 뺀 나머지가 몇이었는지. 둘짜리 평균과 마흔짜리 평균은 무게가 다릅니다. */
  themeMembers: number | null;
  /**
   ...
   * 이 등급이 과거에 실제로 어땠는가.
   *
   * 여기 숫자는 **종가 매수·익일 시가 매도**를 잰 값입니다. 실제 짝꿍매매는 장중에
   * 들어가 장중에 나오므로 같은 자리를 재긴 했어도 보유구간이 다릅니다.
   *
   * 2026-09-02에 분봉으로 그 구간을 쟀고 **성립하지 않았습니다** — 1등주가 잠긴 뒤
   * 2등주가 간격 6%p 안으로 붙은 순간 매수, 12개 장 314건, 마감까지 평균 −1.47%·
   * 상회 38%. 같은 분에 ±3%p 안으로 비슷하게 올라 있던 무관한 종목(29,237건 +0.06%)
   * 보다 나쁩니다(`scripts/measure-pair-entry.mjs`).
   *
   * **장중에는 등급 순서가 뒤집힙니다** — 0~2%p −3.84% · 2~4%p −1.72% · 4~6%p −1.23%.
   * 익일 시가 기준에서 밀착이 최고인 것과 반대이고, 장중에 밀착은 이미 다 갔다는
   * 뜻이기 때문입니다.
   *
   * 그러므로 이 값은 **장중 진입의 근거가 아닙니다.** 화면에도 그렇게 적어야 합니다 —
   * 등급 옆 성적만 보이고 기준이 없으면 지금 들어가도 저 숫자가 나온다고 읽힙니다.
   */
  measured: {
    beatRate: number;
    excessMean: number;
