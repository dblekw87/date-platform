...
export default function KoreanHome() {
  return (
    <main>
      <section className={styles.hero} aria-labelledby="kr-home-title">
        <div className={styles.heroCopy}>
          <p className={styles.eyebrow}>오늘 시장 한눈에 보기 · 예시 화면</p>
          <h1 id="kr-home-title">오늘은 반도체 관련 공식 정보를 먼저 확인해야 합니다.</h1>
          ...
          <div className={styles.heroActions}>
            ...
            <Link href="/kr/stock/005930">관련 종목 보기</Link>
          </div>
        </div>
      ...
      <section className={styles.marketStrip} aria-labelledby="market-title">
        <div className={styles.sectionHeader}>
          ...
          <h2 id="market-title">주요 지수·선물·환율·유가</h2>
        </div>
      ...
      <section className={styles.todayFocus} aria-labelledby="focus-title">
        <div className={styles.sectionHeader}>
          <p className={styles.eyebrow}>오늘 꼭 확인할 변화</p>
          <h2 id="focus-title">핵심 변화 3개</h2>
          <p>등락이 아니라 공식 정보와 아직 확인되지 않은 내용을 함께 봅니다.</p>
        </div>
      ...
      <section className={styles.twoColumn} aria-label="테마와 많이 보는 종목">
        <div className={styles.themeFlow}>
          <div className={styles.sectionHeader}>
            <p className={styles.eyebrow}>주목받는 테마</p>
            <h2>시장 변화에서 이어지는 테마</h2>
          </div>
          <div className={styles.themeTabs} aria-label="테마 목록">
            {themes.map((theme) => (
              <button aria-pressed={theme.name === "반도체"} key={theme.name} type="button">
                {theme.name}
              </button>
            ))}
          </div>
        ...
        <div className={styles.stockList}>
          <div className={styles.sectionHeader}>
            <p className={styles.eyebrow}>많이 보는 종목</p>
            <h2>움직인 이유와 함께 보기</h2>
          </div>
          <ul className={styles.stockRows}>
            {watchedStocks.map((stock) => (
              <li key={stock.code}>
                <Link href={stock.code === "005930" ? "/kr/stock/005930" : stock.code === "AAPL" ? "/kr/stock/AAPL" : "/kr/market"}>
      ...
      <section className={styles.reasonSection} aria-labelledby="reason-title">
        <div className={styles.sectionHeader}>
          <p className={styles.eyebrow}>움직인 이유</p>
          <h2 id="reason-title">시장 변화가 종목과 어떻게 연결되는지 확인하세요.</h2>
        </div>
      ...
      <section className={styles.evidenceSection} aria-labelledby="evidence-title">
        <div className={styles.sectionHeader}>
          <p className={styles.eyebrow}>공식적으로 확인된 투자 근거</p>
          <h2 id="evidence-title">출처와 공개 시각을 먼저 확인합니다.</h2>
        </div>
      ...
      <section className={styles.watchlistSection} aria-labelledby="watch-title">
        <div className={styles.sectionHeader}>
          <p className={styles.eyebrow}>관심 종목에서 달라진 내용</p>
          <h2 id="watch-title">새롭게 감지된 변화를 먼저 봅니다.</h2>
        </div>
        <ul className={styles.simpleList}>
      ...
      <section className={styles.twoColumnBottom} aria-label="이어서 분석과 최근 판단 기록">
        <article>
          <p className={styles.eyebrow}>이어서 분석</p>
          <h2>반도체 공급망 분석을 이어서 볼 수 있습니다.</h2>
          <p>비교 중인 근거: EV-117 · EV-104</p>
          <Link href="/kr/analysis">분석 열기</Link>
        </article>
        <article>
          <p className={styles.eyebrow}>최근 판단 기록</p>
          <h2>이전 판단에서 달라진 근거가 있습니다.</h2>
